...
class TaskRepository:
    ...
    def create_task(self, title: str, description: str | None, status: str,
                    proj_id: int, assigned_to: int | None, deadline: datetime.date | None) -> int | None: # Создать задачу
    ...
    @log
    def get_tasks_by_user(self, assigned_to: int) -> list[Task]: # Получить все задачи пользователя
    ...
    @log
    def assign_task(self, user_id: int, task_id: int) -> int | None: # Назначить задачу пользователю
    ...
    @log
    def update_task_by_id(self, task_id: int, status: str) -> int | None: # Обновить статус задачи
