...
async def response_to_invitation_endpoint(invitation_id: int, invitation_date: InvitationStatusUpdate, current_user: UserModel = Depends(current_user), db: AsyncSession = Depends(get_db)): # Ответ на приглашение
...
async def user_invitations_endpoint(current_user: UserModel = Depends(current_user), db: AsyncSession = Depends(get_db)): # Список приглашений для текущего пользователя
...
async def project_invitations_endpoint(project_id: int, current_user: UserModel = Depends(current_user), db: AsyncSession = Depends(get_db)): # Список приглашений для текущего проекта
...
async def delete_invitation_endpoint(invitation_id: int, current_user: UserModel = Depends(current_user), db: AsyncSession = Depends(get_db)): # Удаление приглашения (только админ)
