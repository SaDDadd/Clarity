...
async def test_getting_project_lets_project_participant_see_details(async_client, member_auth_headers, \
    ...
    response = await async_client.get(f'/api/v1/projects/{project_id}', headers=headers)
    assert response.status_code == 200
...
async def test_updating_not_existent_project(async_client, auth_headers):
    headers = auth_headers
    response = await async_client.put('/api/v1/projects/99999', json={}, headers=headers)
    assert response.status_code == 404
    assert response.json()['detail'] == 'Проект не найден!'
...
async def test_deleting_not_existent_project(async_client, member_auth_headers):
    headers = member_auth_headers
    response = await async_client.delete('/api/v1/projects/99999', headers=headers)
    assert response.status_code == 404
    assert response.json()['detail'] == 'Проект не найден!'
...
@pytest.mark.asyncio
async def test_admin_not_add_user_already_in_project(async_client, auth_headers, test_project_with_member, test_users):
    headers = auth_headers
...
@pytest.mark.asyncio
async def test_admin_raise_or_lower_role(async_client, auth_headers, test_project_with_member, test_users):
    admin_headers = auth_headers
    project = test_project_with_member['project']
    member = test_users.get('member')
    response = await async_client.patch(
        f'/api/v1/projects/{project.project_id}/members/{member.user_id}/role',
        params={'role': 'admin'},
        headers=admin_headers
    )
    assert response.status_code == 200
...
@pytest.mark.asyncio
async def test_not_demote_only_admin(async_client, auth_headers, test_project_with_member, test_users):
    admin_headers = auth_headers
    project = test_project_with_member['project']
    admin_user = test_users.get('admin')
    response = await async_client.patch(
        f'/api/v1/projects/{project.project_id}/members/{admin_user.user_id}/role',
        params={'role': 'member'},
        headers=admin_headers
    )
    assert response.status_code == 403
    assert response.json()['detail'] == 'Нельзя изменить свою собственную роль'
...
@pytest.mark.asyncio
async def test_regular_member_change_role(async_client, member_auth_headers, test_project_with_member, test_users):
    headers = member_auth_headers
    project = test_project_with_member['project']
    member = test_users.get('member')
    response = await async_client.patch(
