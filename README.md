...
Для тестирования API используется **pytest** с асинхронной поддержкой и **httpx**. Все тесты находятся в директории `tests/` и покрывают основные сценарии работы с эндпоинтами, а также репозитории.

Общее количество тестов: **213** — все успешно проходят.
...
- **Тестирование сервисов** — в директории `tests/test_services/` уже заведены заготовки. Планируется покрыть тестами всю бизнес-логику (сервисный слой).
