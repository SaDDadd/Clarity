...
| **Отмена приглашения** (DELETE `/invitations/{invitation_id}`) | ❌ | ✅ (только если он — пригласивший) |
...
Для тестирования API используется **pytest** с асинхронной поддержкой и **httpx**. Все тесты находятся в директории `tests/` и покрывают основные сценарии работы с эндпоинтами.

Общее количество тестов: **103** — все успешно проходят.
...
└── test_api/
    ├── test_auth.py          # Тесты аутентификации (17 тестов)
    ├── test_projects.py      # Тесты проектов (32 теста)
    ├── test_tasks.py         # Тесты задач (27 тестов)
    ├── test_invitations.py   # Тесты приглашений (21 тест)
    └── test_profile.py       # Тесты профиля (12 тестов)
```
...
- **Тестирование репозиториев и сервисов** — в директориях `tests/test_repositories/` и `tests/test_services/` уже заведены заготовки. Планируется покрыть тестами всю бизнес-логику и слой работы с данными.
