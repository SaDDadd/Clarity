...
- **Начало работы над тестами** – в папке `tests` подготовлен `conftest.py` с фикстурами для интеграционного тестирования (тестовые сценарии в разработке).
...
- **pytest** + **httpx** – для тестирования (в разработке)
...
- **Утилиты (Utils)** – вспомогательные функции, декораторы.
- **Тесты (Tests)** – интеграционные и модульные тесты (подготовлена инфраструктура).
...
Для тестов используется отдельный файл `.env.test` с настройками для тестовой базы данных (по умолчанию `task_to_do_test`).
...
│   ├── __init__.py
│   ├── base.py                    # Базовый репозиторий (CRUD) – пока пустой
│   ├── user_repository.py
...
│   └── logger_setuo.py            # (в будущем) настройка логирования – пока пустой
├── tests/                         # Тесты
│   ├── __init__.py
│   ├── conftest.py                # Фикстуры для тестов (движок БД, клиент, сессия)
│   ├── test_repositories/         # Тесты репозиториев (планируется)
│   │   └── __init__.py
│   ├── test_auth.py               # Тесты аутентификации (планируется)
│   ├── test_invitations.py        # Тесты приглашений (планируется)
│   ├── test_profile.py            # Тесты профиля (планируется)
│   ├── test_projects.py           # Тесты проектов (планируется)
│   └── test_tasks.py              # Тесты задач (планируется)
├── .env                           # Переменные окружения (не в репозитории)
├── .env.test                      # Переменные для тестовой БД (не в репозитории)
├── .gitignore
...
- [ ] **Юнит-тесты** – pytest + httpx (инфраструктура уже заложена).
