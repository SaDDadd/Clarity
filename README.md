...
- [API Документация](#-api-документация)
  ...
  - [Проекты](#проекты)
  - [Задачи](#задачи)
  - [Приглашения (в разработке)](#приглашения-в-разработке)
...
> **В разработке:** приглашения в проекты (структура создана, эндпоинты готовятся), чат проекта, уведомления, фильтрация задач, процент готовности проекта, контейнеризация (Docker), логирование и юнит-тесты.
...
### 📨 Приглашения (в разработке)

Эндпоинты для приглашений находятся в стадии реализации. Структура (модель, репозиторий, сервис, роутер) уже создана, но методы пока не дописаны. Планируемые эндпоинты:

- `POST /projects/{project_id}/invitations` – отправить приглашение (только админ)
- `GET /invitations` – список приглашений для текущего пользователя
- `GET /projects/{project_id}/invitations` – список приглашений проекта (только админ)
- `PATCH /invitations/{invitation_id}` – принять/отклонить приглашение
- `DELETE /invitations/{invitation_id}` – отменить приглашение (только админ)
...
│       └── invitations.py   # Роутеры приглашений (в разработке)
├── core/
...
- [ ] **Приглашения в проекты** – администратор приглашает пользователей, те принимают/отклоняют, автоматическое добавление (в процессе).
