...
async def response_to_invitation(db, invitation_id: int, action: InvitationRole): # Возращает информацию о принятии/отклонении приглашения в проект
    repo = ProjectInvitationRepository(db)
    if action != 'accepted':
        raise
...
async def cancel_invitation(db, invitation_id: int, current_user_id: int): # Удалить приглашение
    repo = ProjectInvitationRepository(db)
    repo_proj = ProjectRepository(db)
    if repo.get_invitation_by_id(invitation_id) is None:
        raise
